...
describe("Available Flashcards", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(userIdHelper.getNumericUserId).mockResolvedValue(
      mockNumericUserId
    );
    vi.mocked(userStudyHelper.checkUserHasStudiedRecently).mockResolvedValue(
      false
    );
    vi.mocked(userStudyHelper.updateUserHasStudied).mockResolvedValue(
      undefined
    );
  });
  ...
  describe("fetchAvailableHiragana", () => {
      ...
      (supaClient.from as any).mockImplementation((table: string) => {
        if (table === "studied_flashcards") {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() =>
                  Promise.resolve({ data: mockStudiedFlashcards, error: null })
                ),
              })),
            })),
          };
        ...
        if (table === "hiragana") {
          return {
            select: vi.fn(() => ({
              then: vi.fn(() =>
                Promise.resolve({ data: mockHiraganaData, error: null })
              ),
            })),
          };
        }
    ...
    it("should return message when user has studied recently and no cards are due (Priority 3)", async () => {
      ...
      expect(result.message).toBe(
        "You have finished studying for now! Come back later :)"
      );
    });
